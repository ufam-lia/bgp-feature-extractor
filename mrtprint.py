...
def print_mrt(m):
    ...
    put_lines(
        'Timestamp: %d(%s)' % (m.ts, dt.datetime.fromtimestamp(m.ts)),
...
def print_td(m):
    ...
    put_lines(
        ...
        'Originated Time: %d(%s)' %
        (m.td.org_time, dt.datetime.fromtimestamp(m.td.org_time)),
    ...
    if m.subtype == TD_V2_ST['PEER_INDEX_TABLE']:
        ...
        for entry in m.rib.entry:
            ...
            put_lines(
                ...
                'Originated Time: %d(%s)' %
                (entry.org_time, dt.datetime.fromtimestamp(entry.org_time))
            )
...
def print_bgp_attr(attr, n):
    ...
    if attr.type == BGP_ATTR_T['ORIGIN']:
        ...
        put_lines(line + ': %        sys.stdout.write(contents)d' % attr.med)
