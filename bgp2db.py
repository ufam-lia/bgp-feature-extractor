...
class Metrics(object):
    ...
    def __init__(self):
        ...
        self.conn = mysql.connector.connect(user='pc', database='bgp')
    ...
    def classify_announcement(self, m):
        ...
        for nlri in m.bgp.msg.nlri:
            ...
            #Insert into DB
            add_bgp_msg = ("INSERT INTO Msgs(ts, peer, type_msg, prefix, as_path, origin, " +
                            "next_hop, med, aggregate, aggregator, community) " +
                            "VALUES (%(ts)s, %(peer)s, %(type_msg)s, %(prefix)s," +
                            " %(as_path)s, %(origin)s, %(next_hop)s, %(med)s, "+
                            "%(aggregate)s, %(aggregator)s, %(community)s)")

            msg_attr = dict()
            msg_attr['ts'] = 123
            msg_attr['peer'] = '123'
            msg_attr['type_msg'] = 123
            msg_attr['prefix'] = '123'
            msg_attr['as_path'] = '123'
            msg_attr['origin'] = 1
            msg_attr['next_hop'] = '123'
            msg_attr['med'] = 1
            msg_attr['aggregate'] = 1
            msg_attr['aggregator'] = '123'
            msg_attr['community'] = '123'

            # Insert new employee
            self.cursor.execute(add_bgp_msg, msg_attr)
